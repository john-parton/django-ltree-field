...
class TestLTreeFunctions(TestCase):
    ...
    def test_subpath_function_three_args(self):
        ...
        result = SimpleNode.objects.filter(
            ...
            sub_path=Subpath("path", Value(1), Value(2))
        ).values_list("sub_path", flat=True).first()
        ...
        result = SimpleNode.objects.filter(
            ...
            sub_path=Subpath("path", Value(0), Value(-1))
        ).values_list("sub_path", flat=True).first()
    ...
    def test_nlevel_function(self):
        ...
        results = SimpleNode.objects.filter(
            path__in=["Top", "Top.Collections", "Top.Collections.Pictures.Astronomy"]
        ).annotate(
            depth=NLevel("path")
        ).values_list("path", "depth").order_by("path")
    ...
    def test_index_function_two_args(self):
        ...
        ).values_list("idx", flat=True).first()
        ...
        result = SimpleNode.objects.filter(
            ...
            idx=Index("path", Value("NonExistent"))
        ).values_list("idx", flat=True).first()
    ...
    def test_index_function_three_args(self):
        ...
        result = SimpleNode.objects.filter(
            ...
            idx=Index("path", Value("Astronomy"), Value(2))
        ).values_list("idx", flat=True).first()
    ...
    def test_lca_function(self):
        ...
        nodes = list(SimpleNode.objects.filter(
            path__in=[
                "Top.Collections.Pictures.Astronomy.Stars",
                "Top.Science.Astronomy"
            ]
        ...
        result = SimpleNode.objects.filter(
            ...
            common_ancestor=LCA("path", Value(nodes[1]))
        ).values_list("common_ancestor", flat=True).first()
    ...
    def test_lca_with_array(self):
        ...
        result = SimpleNode.objects.filter(
            ...
            common_ancestor=LCA(
                Cast(
                    Value(paths),
                    output_field=ArrayField(LTreeField())
                )
            )
        ).values_list("common_ancestor", flat=True).first()
    ...
    def test_concat_function(self):
        ...
        result = SimpleNode.objects.filter(
            ...
            new_path=Concat("path", Value("NewSection"))
        ).values_list("new_path", flat=True).first()
