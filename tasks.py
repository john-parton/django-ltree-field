...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-ltree-field runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-ltree-field.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_ltree_field")
...
def lint(c):
    ...
    """
    c.run("flake8 django-ltree-field tests")
...
def release(c, bumpsize=''):
    ...
    import django_ltree_field
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_ltree_field.__version__))
    c.run("git push --tags")
